...
// Sync Generative Function. Not everry generative function is always Async, generative functions can be sync also.
// Unlike Async generative functions, Sync generative functions use Iterable(Parent class  of all collections) class in RDT, use sync* to tell that it is a sync generative functions.
// In the body for loop iterate and every time value of i will flow in a stream one by one, but all the process occur so fast without delay that we get complete Iterable as sson as we call this sync generative functions.
